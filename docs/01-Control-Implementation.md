...
<div align="center">
	<table>
      ...
      <td align="left"> 
        Sampling Frequency: 1000 Hz (~161 times motor bandwidth) <br>
        Time-Sampling: 1 ms
      </td>
    </tr>    
		<tr>  
...
After we got the estimation bandwidth of the DC motor, we can decided how much the time-sampling that we want to apply for the firmware controller. For this project I decided to use `1000 Hz` sampling frequency because it's not consuming much CPU usage with good safety factor (around `161 times` the DC motor bandwidth). Another method that we can use is how much data to reach the time-constant. For the 0.025 of time-constant, we can get 25 data with 1000 Hz time-sampling. This value is enough for experimental purpose. After we get the time-sampling, we can design the firmware based on this value.
